...
export class MypageModel {
  ...
  async findBookmarkedContestsByUserId(userId: number) {
    ...
    try {
      conn = await getDBConnection().getConnection();
      const query = `
        SELECT c.contest_id as id, c.title, c.organizer, c.end_date as endDate
        FROM scrap s
  ...
  async findBookmarkedCommunitiesByUserId(userId: number) {
    ...
    try {
      conn = await getDBConnection().getConnection();
      const query = `
        SELECT co.community_id as id, co.title, co.content, co.reg_date as createdDate, co.community_type as communityType
        FROM scrap s
